  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      const token = jwt.sign(
        { id: user.id, username: user.username, role: user.role, designation: user.designation },
        SECRET,
        { expiresIn: "1d" }
      );
      ...
      const [rows] = await db.query(`
        SELECT 
          CASE 
            ...
            ELSE 'Night'
          END as time_period,
          SUM(CASE WHEN FC='Y' THEN 1 ELSE 0 END) as forecasted,
          SUM(CASE WHEN IC='Y' THEN 1 ELSE 0 END) as actual
        FROM \`${table}\`
        WHERE \`DUE DATE\` BETWEEN CURDATE() - INTERVAL 1 DAY AND CURDATE() + INTERVAL 1 DAY
